...
from utils.rotation import transform_to_matrix, RotType
...
def mpjpe(pred, gt, root_index, normalize=True):
    ...
    if normalize is False:
        dist = np.where(valid, dist, -1.0)
        return dist
    else:
        dist = np.where(valid, dist, 0.0)
        norm = float(np.sum(valid > 0.0))
...
def nmpjpe(pred, gt, root_index, alignment="root", normalize=True):
    ...
    if alignment == "mean":
        ...
        mean_gt3d = (
            np.sum(gt3d * valid[:, :, np.newaxis], axis=1) / normalizer[:, np.newaxis]
        )
        gt3d = gt3d - mean_gt3d[:, np.newaxis, :]

        mean_pred3d = (
            np.sum(pred * valid[:, :, np.newaxis], axis=1) / normalizer[:, np.newaxis]
        )
...
def pmpjpe(pred, gt, normalize=True):
    ...
    for p, g in zip(pred, gt3d):
        try:
            _, p_aligned, _, _, _ = compute_similarity_transform(
                X=g, Y=p, compute_optimal_scale=True
            )
            aligned_pred.append(p_aligned)
        except np.linalg.LinAlgError as e:
            logging.info("Warning: SVD did not converge during PAMPJPE")
...
def optimal_scaling(pred3d, target3d, valid_mask):
    masked_target, masked_pred = (
        target3d * valid_mask[:, :, np.newaxis],
        pred3d * valid_mask[:, :, np.newaxis],
    )
    # Calculate scales
    nom = (
        np.sum(masked_pred[:, :, 0] * masked_target[:, :, 0], axis=1)
        + np.sum(masked_pred[:, :, 1] * masked_target[:, :, 1], axis=1)
        + np.sum(masked_pred[:, :, 2] * masked_target[:, :, 2], axis=1)
    )
    denom = (
        np.sum(masked_pred[:, :, 0] * masked_pred[:, :, 0], axis=1)
...
def joint_angle_error(pred: np.ndarray, gt: np.ndarray, average: bool = True) -> float:
    """Compute the geodesic distance between the sequence of rotatations matrices.

    Args:
        pred (np.ndarray): predicted rotations in format (B, K, R).
        gt (np.ndarray): ground truth rotations in format (B, K, R).

    Returns:
        float: Mean geodesic distance between input matrices in radians.
    """
    eps = 1e-7
    ...
    rotation_dim = pred.shape[-1]
    rot_type = RotType(rotation_dim)
    # if rot_type == RotType.ROT_MAT:
    #     pred_m = symmetric_orthogonalization(pred)
    #     gt_m = gt.clone()
    # else:
    pred_m = transform_to_matrix(pred, rot_type)
    gt_m = transform_to_matrix(gt, rot_type)

    # Compute the relative rotation matrices
    R_rel = torch.matmul(pred_m, gt_m.transpose(-1, -2))
    ...
    theta_rad = torch.acos(torch.clamp((trace_R_rel - 1) / 2, -1 + eps, 1 - eps))

    # Convert theta from radians to degrees
    theta_deg = torch.rad2deg(theta_rad)

    if average:
        mean_theta = torch.mean(theta_deg).item()
    else:
        # mean_theta = torch.mean(theta_deg, axis=0).detach().cpu().numpy()
        mean_theta = theta_deg.detach().cpu().numpy()
